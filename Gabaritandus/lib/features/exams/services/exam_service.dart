...
class ExamService {
  ...
  Future<String?> _getGroupToken() async {
    ...
    if (token == null) {
      ...
      print("🔐 [ExamService] Token do grupo encontrado (${token.length} chars)");
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map && data["message"] == "Exams fetched successfully") {
          final exams = List<Map<String, dynamic>>.from(data["data"]["exams"] ?? []);
        ...
        print("❌ [ExamService] Erro HTTP ${response.statusCode}: $responseBody");
